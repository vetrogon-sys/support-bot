                        ...
                        message.hasText() -> {
                            ...
                            when {
                                ...
                                text.startsWith("/status") -> {
                                    ...
                                    if (ticketId.isBlank()) {
                                        sendResponse(chatId, "Please provide a ticket ID with the status command. For example: /status 12345")
                                    } else {
                                        val statusResponse = messageProcessor.getTicketStatus(ticketId)
                                ...
                                text.startsWith("/") -> {
                                    sendResponse(chatId, "Unknown command. Type /help for available commands.")
                                }
                                else -> {
                            ...
                            sendResponse(update.message.chatId.toString(), "Unsupported message type")
                        }
                    }
                }
            } catch (e: Exception) {
